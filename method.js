...
storeItem.forEach(storeitems) // forEach  expect a callback function and inside it lies item, value and array in that order
...
//----------------------------------------------every and some ------------------------------------------------------
//                                                some
//iterates over our array to check if a  given condition is true for at least one of the values if it is
// it returns true but if no value passes the test then it returns false 
let numbers=[1, 2, 3, 4, 7, 9]
let num=numbers.some(function(items){
  return items > 9
})
console.log("this num",num) // this will return false because there is no number that is greater than 9

let Numbers=[1, 2, 3, 4, 7, 9,10]
let Num=numbers.some(function(items){
  return items > 9 
  // if (items > 9){
  //   return items
  // } this will always only evaluate to a bollean not the number
})
console.log("this Num",Num) // this will return true because there at least one number that is greater than 9

//similar to the sum method however now every value needs to meet that condition then and only then will it return true but if every value doesn't meet that conditionthen it will turn false 

Enumber=numbers.every(function(items){
  return items > 9 

})
console.log("EveryNumber",Enumber) // this will return false because not all the numbers in the array is is greater than 9